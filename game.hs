...
initialMonster2 :: Monster
initialMonster2 = Monster (100, 100) (Wander WalkUp wanderDist) 
 
initialMonster3 :: Monster
initialMonster3 = Monster (100, -205) (Wander WalkUp wanderDist) 
...
main = do
    ...
    withWindow width height "PacMan Beta" $ \win -> do
...
hunted win directionKey randomGenerator glossState = mdo
    ...
    randomNumber2 <- stateful (undefined ,randomGenerator) nextRandom

    randomNumber3 <- stateful (undefined ,randomGenerator) nextRandom

    monster <- transfer3 initialMonster wanderOrHunt player randomNumber gameOver'

    monster2 <- transfer3 initialMonster2 wanderOrHunt player randomNumber2 gameOver'

    monster3 <- transfer3 initialMonster3 wanderOrHunt player randomNumber3 gameOver'
    ...
    return $ renderFrame win glossState <$> player <*> monster <*> monster2 <*> monster3 <*> gameOver
...
renderFrame :: Window
               ...
               -> State -> Player -> Monster -> Monster ->Monster -> Bool -> IO ()

renderFrame window glossState (Player (xpos, ypos)) (Monster (xmon, ymon) status) (Monster (xmon2, ymon2) status2) (Monster (xmon3, ymon3) status3) gameOver = do
   ...
   displayPicture (width, height) black glossState 1.0 $ 
     ...
     Pictures $ gameOngoing gameOver
                             ...
                             [renderPlayer xpos ypos,
                              ...
                              renderMonster2 status2 xmon2 ymon2,
                              renderMonster3 status3 xmon3 ymon3]
...
renderPlayer xpos ypos = Color white $ translate xpos ypos $ rectangleSolid playerSize playerSize
...
renderMonster2 :: MonsterStatus -> Float -> Float -> Picture

renderMonster2 status xpos ypos = Color (monsterColor status)  $ translate xpos ypos $ circleSolid playerSize

                           where monsterColor Hunting = red

                                 monsterColor (Wander _ _) = green

renderMonster3 :: MonsterStatus -> Float -> Float -> Picture

renderMonster3 status xpos ypos = Color (monsterColor status)  $ translate xpos ypos $ circleSolid playerSize
